...
function createGridItems (size) {
    ...
    for (let i = 0; i< size*size ; i++) {
        ...
        gridItem.addEventListener("mouseenter", () => {
            switch (currentMode) {
                case "color":
                    gridItem.style.cssText="background-color:"+selectedColor.value+"; border: 0px; border-radius: 0px";
                    break;
                case "rainbow":
                    let r = Math.floor(Math.random() *256);
                    let g = Math.floor(Math.random() *256);
                    let b = Math.floor(Math.random() *256);
                    gridItem.style.cssText="background-color:rgb("+r+","+g+","+b+"); border: 0px; border-radius: 0px";
                    break;
                case "eraser":
